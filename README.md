...
Quick start (local)
Clone or create the project locally (you will create the GitHub repo later):
...
In Firebase Console > Project settings > Service accounts, generate a new private key for a service account and download it. Save it as: src/seed/serviceAccountKey.json

Update DATABASE_URL in src/seed/seed.js if needed (should match your Realtime Database URL).

Install firebase-admin (if not installed above):
...
(Optional) Add environment variables in Vercel if you moved firebase config to env vars.
